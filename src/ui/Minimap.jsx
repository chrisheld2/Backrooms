...
import { GRID_W, GRID_H, HZ_HUD, worldToCellX, worldToCellY } from '../game/config.js';
...
export default function Minimap() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      const pcx = worldToCellX(player.x);
      const pcy = worldToCellY(player.z);
      const originX = pcx - RADIUS;
      const originY = pcy - RADIUS;
      ...
      if (px && pz) {
        for (let i = 0; i < px.length; i++) {
          ...
          const gx = worldToCellX(px[i]) - originX;
          const gy = worldToCellY(pz[i]) - originY;
          if (gx < 0 || gx >= SPAN || gy < 0 || gy >= SPAN) continue;
          ctx.fillStyle = '#cdf3ff';
          ctx.beginPath();
          ctx.arc((gx + 0.5) * cell, (gy + 0.5) * cell, cell * 0.28, 0, Math.PI * 2);
          ctx.fill();
        }
      }
      ...
      // Exit, once unlocked
      if (world.exitOpen) {
        const gx = worldToCellX(world.exitX) - originX;
        const gy = worldToCellY(world.exitZ) - originY;
        if (gx >= 0 && gx < SPAN && gy >= 0 && gy < SPAN) {
          ctx.fillStyle = '#7be07b';
          ctx.beginPath();
          ctx.arc((gx + 0.5) * cell, (gy + 0.5) * cell, cell * 0.34, 0, Math.PI * 2);
          ctx.fill();
        }
      }
      ...
      if (entity.active && entity.hunting) {
        const gx = worldToCellX(entity.x) - originX;
        const gy = worldToCellY(entity.z) - originY;
        if (gx >= 0 && gx < SPAN && gy >= 0 && gy < SPAN) {
          ctx.fillStyle = '#c8452e';
          ctx.beginPath();
          ctx.arc((gx + 0.5) * cell, (gy + 0.5) * cell, cell * 0.32, 0, Math.PI * 2);
          ctx.fill();
        }
      }
